...
export default async function movimientosRoutes(app) {
  ...
  // Coincide con los CHECK de la tabla movimientos (tipo_m, medio_pago_m).
  const TIPO_M = ['Ingreso', 'Egreso']
  const MEDIO_PAGO_M = ['Efectivo', 'Debito', 'Transferencia', 'Credito', 'Otro']
  ...
  app.get('/resumen', admin, async (req) => {
  ...
  app.get('/balance-mensual', admin, async () => {
  ...
  app.get('/categorias', admin, async () => {
    const { rows } = await query(`
  ...
  app.post('/', {
    ...
    schema: {
      body: {
        type: 'object',
        required: ['id_categoria', 'tipo_m', 'monto_m'],
        properties: {
          id_categoria: { type: 'integer' },
          tipo_m: { type: 'string', enum: TIPO_M },
          monto_m: { type: 'number', exclusiveMinimum: 0 },
          descripcion_m: { type: ['string', 'null'] },
          medio_pago_m: { type: 'string', enum: MEDIO_PAGO_M },
          fecha_m: { type: 'string', format: 'date' },
        }
      }
    }
  }, async (req, reply) => {
